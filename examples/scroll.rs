...
impl ScrollExample {
    ...
    fn view(&self, ctx: &Context, state: ScrollState) -> Node {
        ...
        let palette = [
            Color::Rgb(0x13, 0x3a, 0x63),
            Color::Rgb(0x1f, 0x4d, 0x38),
            Color::Rgb(0x59, 0x26, 0x4f),
            Color::Rgb(0x37, 0x41, 0x52),
        ];
        let horizontal_cards: Vec<Node> = (1..=8)
            .map(|i| {
                let color = palette[(i as usize - 1) % palette.len()];
                node! {
                    div(bg: color, w: 18, h: 6, pad: 1, border: bright_black, border_style: rounded) [
                        text(format!("Panel {i:02}"), color: bright_white, bold),
                        spacer(1),
                        text("Use ←/→ or Shift + scroll to pan.", color: bright_white)
                    ]
                }
            })
            .collect();
        ...
        node! {
            div(
                ...
                text("Horizontal gallery", color: bright_yellow, bold),
                div(
                    bg: "#111317",
                    border: bright_white,
                    pad: 1,
                    ...
                    show_scrollbar: false,
                    @focus: ctx.handler(ScrollMsg::SetHint(
                        "Horizontal gallery: use Left/Right arrows or Shift + mouse wheel to pan sideways."
                    )),
                    @blur: ctx.handler(ScrollMsg::ResetHint)
                ) [
                    ...(horizontal_cards)
