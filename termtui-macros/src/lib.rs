...
pub fn derive_component(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl termtui::Component for #name {
            fn get_id(&self) -> Option<termtui::ComponentId> {
                self.#id_field.clone()
            }

            fn set_id(&mut self, id: termtui::ComponentId) {
            ...
            fn clone_box(&self) -> Box<dyn termtui::Component> {
                Box::new(self.clone())
            }

            // Forward to the user's implementations
            fn update(&self, ctx: &termtui::Context, msg: Box<dyn termtui::Message>, topic: Option<&str>) -> termtui::Action {
                <#name>::update(self, ctx, msg, topic)
            }

            fn view(&self, ctx: &termtui::Context) -> termtui::Node {
                <#name>::view(self, ctx)
            }
        }
    };
